...
class ClassTranslator private constructor(
    ...
    private fun generateClassCreateInvocationArguments() {
        ...
        val constructorFunction = context.addRootFunction(descriptor)
        ...
        for (parameter in getPrimaryConstructorParameters(classDeclaration)) {
            ...
            if (descriptor != null) {
                ...
                if (literal.propertyInitializers.isNotEmpty()) {
                    addAccessorsToPrototype(this.descriptor, descriptor, literal, classDeclarationContext)
                }
            }
        }
    }
    ...
    private fun addInterfaceDefaultMembers() {
        ...
        for (member in members) {
            ...
            if (member.modality == Modality.ABSTRACT) continue
    ...
    private fun addDefaultMethodFromInterface(function: FunctionDescriptor, interfaceDescriptor: ClassDescriptor) {
        val classPrototype = JsAstUtils.prototypeOf(context().getInnerNameForDescriptor(descriptor).makeRef())
        val interfacePrototype = JsAstUtils.prototypeOf(context().getInnerNameForDescriptor(interfaceDescriptor).makeRef())
    ...
    private fun addObjectMethods() {
        val instanceVarName = context().createGlobalName(ManglingUtils.getSuggestedName(descriptor) + Namer.OBJECT_INSTANCE_VAR_SUFFIX)
        context().addRootStatement(JsAstUtils.newVar(instanceVarName, JsLiteral.NULL))
        ...
        val instanceCreatedCondition = JsAstUtils.equality(instanceVarName.makeRef(), JsLiteral.NULL)
        ...
        val objectName = context().getInnerNameForDescriptor(descriptor)
        instanceCreationBlock.statements += JsAstUtils.assignment(instanceVarName.makeRef(), JsNew(objectName.makeRef())).makeStmt()

        instanceFun.body.statements += JsReturn(instanceVarName.makeRef())
